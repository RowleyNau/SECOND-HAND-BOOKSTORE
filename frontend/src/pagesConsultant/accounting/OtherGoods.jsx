...
import { OtherGoodsGetAll, OtherGoodsSearchGetAll, OtherGoodsGet } from "../../http/otherGoodsManagementApi.js";
// import {OtherGoodsAddStr} from './ModalWinOtherGoods/OtherGoodsAddStr.jsx';
import OtherGoodsAddStr from "./ModalWinOtherGoods/OtherGoodsAddStr.jsx";
// import BookInfo from './ModalWinBooks/BookInfo.jsx';
// import {OtherGoodsInfo} from './ModalWinOtherGoods/OtherGoodsInfo.jsx';
...
const OtherGoods = observer(() => {
    ...
    const BookShow = async(Search)=>{
        ...
        if  (Search & SearchInput!=''){
            ...
            data = await OtherGoodsSearchGetAll(50, booksPage, SearchInput);
        }
        else {
            setSearchStatus(false);
            data = await OtherGoodsGetAll(50, booksPage);
        }
        setBooks(data.good)
    ...
    const BookInfoFun = async(book)=>{
        let data;
        data = await OtherGoodsGet(book);
        console.log(data);
        setBookInfoId(await OtherGoodsGet(book));
    ...
    <div className="mainUnit">
        <PageTitle textTitle="учёт иных товаров"/>
                    ...
                    <tr className='SearchTable'>
                        {
                            SearchStatus? 
                        <>
                        <th colspan="4"><Search ClickBut = {()=>{BookShow(true)}} value={SearchInput} onClick={e =>  setSearchInput(e.target.value)} setValue = {setSearchInput}/></th>
    ...
    { modalBookAdd?
        <Modal active={modalBookAdd} setActive={setModalBookAdd}>
            <OtherGoodsAddStr active={modalBookInfo} setActive={setModalBookInfo}/>
        </Modal>
        :
        <></>
    }    
    { modalBookInfo?
        <Modal active={modalBookInfo} setActive={setModalBookInfo}>
            <OtherGoodsInfo IdGoodInfo={BookInfoId}/> 
        </Modal>
        :
        <></>
    }   
