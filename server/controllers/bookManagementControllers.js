...
const {Goods, BookList, BookAuthors, BookCategories, BookGenres, GoodsSize, Owners, PhotoGoods, Connections, Locations, Availability, Authors, Genres, Categories, Bindingtype, Agerestriction, Goodstates, Clients, Publishers, Cities, GoodsLocations} = require('../models/models')
...
class BookManagementController {
    ...
    async BookAdd(req, res) {
        ...
        console.log((!(Name)||!(Author)||!(Price)||!(IdStates)||!(Amount)||!(IdLocations.length==0)||!(IdAvailability)||!(NumberOfPages)||!(IdBindingType)||!(IdAgeRestriction)||!(Description)||!(Genre)||!(Parametr1)||!(Parametr2)||!(Parametr3)))

        if ((((Name=='')||(Author=='')||(Price=='')||(IdStates=='')||(Amount=='')||(IdAvailability=='')||(NumberOfPages=='')||(IdBindingType=='')||(IdAgeRestriction=='')||(Description=='')||(Genre=='')||(Parametr1=='')||(Parametr2=='')||(Parametr3==''))||(!(Name)||!(Author)||!(Price)||!(IdStates)||!(Amount)||(IdLocations.length==0)||!(IdAvailability)||!(NumberOfPages)||!(IdBindingType)||!(IdAgeRestriction)||!(Description)||!(Genre)||!(Parametr1)||!(Parametr2)||!(Parametr3)))||!MainPhoto){
        ...
        try {
            const good = await Goods.findOne({
                ...
                include: [
                    ...
                    {
                        model: GoodsLocations,
                        attributes: ['IdGoodslocations', 'Amount'],
    ...
    async BookUpdate(req, res) {
        ...
        console.log(req.body);
        console.log(((Name=='')||(Author=='')||(Price=='')||(IdStates=='')||(Amount=='')||(IdLocations.length==0)||(IdAvailability=='')||(NumberOfPages=='')||(IdBindingType=='')||(IdAgeRestriction=='')||(Description=='')||(Genre=='')||(Parametr1=='')||(Parametr2=='')||(Parametr3=='')))
        if ((((Name=='')||(Author=='')||(Price=='')||(IdStates=='')||(Amount=='')||(IdLocations.length==0)||(IdAvailability=='')||(NumberOfPages=='')||(IdBindingType=='')||(IdAgeRestriction=='')||(Description=='')||(Genre=='')||(Parametr1=='')||(Parametr2=='')||(Parametr3==''))||(!(Name)||!(Author)||!(Price)||!(IdStates)||!(Amount)||!(IdLocations)||!(IdAvailability)||!(NumberOfPages)||!(IdBindingType)||!(IdAgeRestriction)||!(Description)||!(Genre)||!(Parametr1)||!(Parametr2)||!(Parametr3)))||!MainPhoto){
            console.log(Name, ' | ', Author, ' | ', Price, ' | ', IdStates, ' | ', Amount, ' | ', IdLocations, ' | ', IdAvailability, ' | ', NumberOfPages, ' | ', IdBindingType, ' | ', IdAgeRestriction, ' | ', Description, ' | ', Genre, ' | ', Parametr1, ' | ', Parametr2, ' | ', Parametr3);
            return res.status(400).json(ApiError.badRequest('Не все обязательные поля заполнены'));
            // "Missing required fields"
            }
    ...
    async BookCharacteristicsGetAll(req, res) {
        // const token = req.headers.authorization.split(' ')[1];
        let {ch} = req.query;
        console.log(req.query)
        console.log(req.query);
